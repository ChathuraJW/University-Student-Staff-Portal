...
class GetRawResultModel extends Model{
    public static function loadReviewData(){
        ...
        $fileList=Database::executeQuery("administrativeSAR","administrativeSAR@16",$sqlQuery);
        $returnDataset=Array();
        foreach ($fileList as $row){
            $fileID=$row['fileID'];
            $sqlQuery="SELECT * FROM result_data_file WHERE fileID=$fileID LIMIT 1";
            $fileInfo=Database::executeQuery("administrativeSAR","administrativeSAR@16",$sqlQuery);
//            replace subject code with subject code and name
    ...
    public static function sendResultReceiveConfirmation($fileID){
        ...
        $result=Database::executeQuery("administrativeSAR","administrativeSAR@16",$sqlQuery);
//        create audit trail
