<?php
	...
	class facilityManagementModel extends Model {
		...
		public static function getHall() {
			$hallList = array();
			$query = "SELECT * FROM  hall_and_lab WHERE hallValidity=1";
		...
		public static function editHall($hallID, $capacity, $hallType) {
			...
			if ($hallID == "" || $capacity == "" || $hallType == "") {
				echo("<script>createToast('Warning (error code: #ADMIN-FM-01)','cannot set null failed. Please Try Again.','W');</script>");
			} else {
				...
				if ($databaseInstance->getTransactionState()) {
					...
					//			show failed toast
					echo("<script>createToast('Warning (error code: #ADMIN-FM-02)','Update Query failed. Please Try Again.','W');</script>");
				}
				//		close db connection
		...
		public static function deleteHall($hallID) {
			...
			$query = "UPDATE hall_and_lab SET hallValidity=0 WHERE hallID='" . $hallID . "'";
